...
import { dutyReq, listing_duties_req, staff_by_roles_req, unassign_role_req } from "../../../../redux/staff/LMS/lmsAction";
...
const dutyList = [
  ...
  {name : "counselling Incharge" ,
  roleId: 7},
  {name : "Admission Executive"  ,
  roleId: 8},
  {name : "Verificaton Officer" ,
  roleId: 9},
...
function Home(props) {
  ...
  const columns = [
    { field: 'id',renderHeader:()=><span className="bold"> S.No</span>, headerClassName : "cell hideRightSeparator",headerAlign:"center", width: 122,cellClassName : "cell",align : "center" },
        { field: 'firstName', renderHeader:()=><span className="bold"> Faculty Name</span>, headerClassName : "cell hideRightSeparator",headerAlign:"center", width: 279.,cellClassName : "cell",align : "center" },
        { field: 'duty', renderHeader:()=><span className="bold"> Duty Assigned</span>, headerClassName : "cell hideRightSeparator",headerAlign:"center",width: 279,cellClassName : "cell",align : "center" },
        
        {
          field: 'action',
          renderHeader:()=><span className="bold"> Action</span>,
          headerClassName : "cell hideRightSeparator",
          headerAlign:"center",
          cellClassName : "cell",
          sortable: false,
          width: 279,
          renderCell : (data)=>{
            console.log(data)
            return(<RoleAssingedButton onClick ={()=>{props.unassingn_role(data.value)
              setTimeout(()=>{
                props.Staff_By_Roles({role: "7"})
                props.Staff_By_Roles({role: "8"})
                props.Staff_By_Roles({role: "9"})},500)
            }} />)},
          align : "center",
        },
      ];
      ...
      const [values , setValues] = React.useState({department : "" , faculty :"" , duty:""});
  ...
  useEffect(() => {
    ...
    props.Staff_By_Roles_List.map(i => {
      // console.log(i)
      i.role.map(el =>{
        if((el.roleId === 7 || el.roleId ===  8 || el.roleId ===  9) && el.active === true){
          let data = {id: index++ , 
  ...
  setRows([...setTemp]);
    ...
    <h3 className="card-header" style={{color: "#a93131"}}>Duty Assignment:</h3>
      ...
      {item.name}
        ...
        <FormControl sx={{ m: 1, width: "100%" ,flexBasis: "400px" }} >
          ...
          <Select
            ...
            onChange={e=>setValues({...values , faculty: e.target.value})}
          ...
          <FormControl sx={{ m: 1, width: "100%",flexBasis: "400px" }} >
            ...
            <Button variant="contained"  onClick={()=>{
              props.dutyAssinged({staffId:values.faculty , roleId:values.duty})
              setTimeout(()=>{
              props.Staff_By_Roles({role: "7"})
              props.Staff_By_Roles({role: "8"})
              props.Staff_By_Roles({role: "9"})},500)
            }} style={{width: 100 , display:"flex", justifyContent: "center", margin:"10px"}} >Assign</Button>
        </Card>
...
const RoleAssingedButton = props=>{
  ...
  return (
    <Button 
    style={{backgroundColor: "#a93131"}} size="small" variant="contained" color = "error" onClick={()=>props.onClick()}  > Remove</Button>
  );
}
